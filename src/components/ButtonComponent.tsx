...
const Button: React.FC<ButtonProps> = ({
  ...
  const variantStyles: Record<string, string> = {
    primary: 'bg-gold_primary text-blue_primary transition duration-200 easi-in-out hover:bg-gold_secondary',
    secondary:
      'bg-transparent border border-gray-500 border-opacity-50 text-gold_secondary hover:border-gold_secondary transition duration-200 easi-in-out',
  };

  // Veľkosti tlačidla
  const sizeStyles: Record<string, string> = {
    small: 'px-[32px] py-[12px] text-sm',
    large: 'px-[40px] py-[20px] text-base',
  };
